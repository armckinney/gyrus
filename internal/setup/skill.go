...
// InstallAgentSkill equips the target workspace directory with the Gyrus Agent Skill files.
func InstallAgentSkill(workspaceDir string, target SkillTarget) ([]string, error) {
	skillDirs := []string{
		filepath.Join(workspaceDir, ".agents", "skills", "gyrus"),
		filepath.Join(workspaceDir, "skills", "gyrus"),
	}
